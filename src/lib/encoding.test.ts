import { assertEquals, decodeHexString, encodeHexString } from "../deps.ts";
import { SmppEsmClass, SmppFeatures, SmppMessageType, SmppMessagingMode } from "./esm_class.ts";
import { SmppTlvTag } from "./tlv.ts";
...
Deno.test("BindResponse", async (t) => {
  const pdu = {
    ...
    systemId: "foo-bar",
    tlvs: [{
      tag: SmppTlvTag.ScInterfaceVersion,
...
Deno.test("MessageRequest", async (t) => {
  ...
  const pdu = {
    ...
    esmClass: new SmppEsmClass(
      ...
      new Set([SmppFeatures.SetReplyPath]),
    ),
    protocolId: 0,
    priorityFlag: 0,
    ...
    shortMessage,
    tlvs: [{
      tag: SmppTlvTag.AlertOnMessageDelivery,
      value: new Uint8Array([0x01, 0x02, 0x03, 0x04]),
    }],
  ...
  const expected: HexFormOf<typeof pdu> = {
    ...
    registeredDelivery: "00", // 00000000
