...
export class SmppEsmClass {
  constructor(
    ...
    public features: Set<SmppFeatures>,
  ) {}
  ...
  static fromByte(b: number): SmppEsmClass {
    ...
    const featuresValueSet = new Set<SmppFeatures>();

    if ((featuresMask & SmppFeatures.UdhiIndicator) === SmppFeatures.UdhiIndicator) {
      featuresValueSet.add(SmppFeatures.UdhiIndicator);
    }

    if ((featuresMask & SmppFeatures.SetReplyPath) === SmppFeatures.SetReplyPath) {
      featuresValueSet.add(SmppFeatures.SetReplyPath);
    }
